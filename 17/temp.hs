...
calculateMinCostPath :: Layout -> Int
...
dijkstraCost :: Layout -> Pos -> [Node] -> Int
dijkstraCost layout target queue
  | pos minCostNext == target = cost minCostNext
  | otherwise = dijkstraCost layout target updatedQueue where
    minCostNext = findMinCost queue
    queueWithoutMin = removeFromQueue minCostNext queue
    neighbors = findNeighbors layout minCostNext
    updatedQueue = queueWithoutMin ++ neighbors
...
isFourthInSameDir ns n = fourXs || fourYs where
  nx = fst $ pos n
  ny = snd $ pos n
  poss = map pos ns
...
main = do
  ...
  let minCostPath = calculateMinCostPath layout
  print minCostPath